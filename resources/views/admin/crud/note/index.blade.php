        ...
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">الملحوظه </h5>
...
@section('js')
    ...
    <script>
        ...
        datatable_selector = $('#Datatable').DataTable({
            ...
            "searching": true,
            'iDisplayLength': 20,
            "ajax": "{{route('notes.list',['id'=>$order_id])}}",
            "columns": [
            ...
            $.ajax({
                ...
                success: function (data) {
                    window.setTimeout(function () {
                        ...
                        $('#form-for-addOrDelete').html(data.html);
                        $('.loader-ajax').hide()
                        $('#exampleModalLabel').html('تعديل الملحوظة')
                        $('#exampleModalCenter').modal('show')
                        $('.dropify').dropify()
                        $('[data-toggle="tooltip"]').tooltip()
                        $.validate({
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                console.log(result)
                if (result.value) {
                    var url = '{{ route("notes.destroy", ":id") }}';
                    url = url.replace(':id', id);
                    $.ajax({
                        url: url,
                        type: 'DELETE',
