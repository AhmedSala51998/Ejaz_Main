                        ...
                        <div id="bulk_actions_wrapper" style="display: none;">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu shadow-sm border-0" aria-labelledby="bulkActionsBtn">
                                    <li>
                                        <a class="dropdown-item text-success d-flex align-items-center gap-2" href="#" id="bulk_show">
...
@section('js')
    ...
    <script>
        $(document).on('click', '#bulk_show', function (e) {
            e.preventDefault();
            ...
            Swal.fire({
                title: 'تأكيد إظهار العناصر؟',
                text: "سيتم إلغاء الإخفاء عن العناصر المحددة",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('biographies.toggleHide') }}", // 👈 مهم
                        type: "POST",
                        data: {
                            id: ids,
                            status: 0, // 👈 إلغاء الإخفاء
                            _token: '{{ csrf_token() }}'
                        },
        ...
        $(document).on('click', '.toggle-hide', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('biographies.toggleHide') }}",
                        type: 'POST',
                        data: {
                            id: id,
                            status: status,
