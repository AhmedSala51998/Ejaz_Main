<form action="{{route('recruitment-offices.update',$obj->id)}}" method="post" id="Form">
    @csrf
    @method('PUT')
    ...
    <div class="row ">
        ...
        @endforeach
            ...
            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
                <label for="_title">الدولة</label>

                <select name="country_id" data-validation="required"   required  class="form-select" aria-label="Default select example">
                    @foreach(\App\Models\Nationalitie::get() as $country)
                        <option value="{{$country->id}}" @if($obj->country_id==$country->id)  selected @endif>{{$country->country_name}}</option>
